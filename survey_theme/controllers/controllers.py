...
class SurveyTheme(Survey):
    ...
    @http.route('/survey_background/<int:id>', type='http', auth="none", cors="*")
    def survey_background(self, id=None, **kw):
        X = request.env['survey.survey'].sudo().search([('id','=',id)]).background_image
        image_base64 = base64.b64decode(X)
        image_data = io.BytesIO(image_base64)
        mimetype = guess_mimetype(image_base64, default='image/png')
        imgext = '.' + mimetype.split('/')[1]
        if imgext == '.svg+xml':
            imgext = '.svg'
        response = http.send_file(image_data, filename='survey_img' + imgext, mimetype=mimetype)
        return response
        # imgname = 'logo'
        # imgext = '.png'
